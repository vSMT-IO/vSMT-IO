...
echo "sysbench OLTP mysql, Threads number: $1"
	...
	sysbench --test=oltp --oltp-table-size=1000000 --mysql-db=dbtest --mysql-user=root --mysql-password=kvm prepare > /dev/null
	#./flush > /dev/null
	...
	./flush
	/usr/bin/sysbench --test=oltp --oltp-table-size=1000000 --mysql-db=dbtest --mysql-user=root --mysql-password=kvm --max-time=60 --oltp-read-only=on --max-requests=0 --num-threads=$1 run
	echo "============> sysbench end===============>"
	sysbench --test=oltp --mysql-db=dbtest --mysql-user=root --mysql-password=kvm cleanup > /dev/null
