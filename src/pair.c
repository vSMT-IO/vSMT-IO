...
* pair.c
*
* Weiwei Jia <harryxiyou@gmail.com> (C) 2018
*
* Performance analysis for good pairs.
*
*/
#define _GNU_SOURCE
#define DEBUG_XPAIR_
...
uint64_t get_pid_affinity(uint64_t pid) {
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset);
...
void init_vcpu(void) {
	...
	for (i = 1; i <= vm_num; i++) {
		for (j = 0; j < kvm[i].vcpu_num; j++) {
			...
			printf("affinity is %lu\n", get_pid_affinity(vcpu[num].pid));
			num += 1;
		}
	}
	debug_vcpu();
...
int main(int argc, char **argv) {
	...
	parse_config(argv[1]);
	init_vcpu();
	//printf("affinity is %lu\n", get_pid_affinity(33124));
	//set_pid_affinity(0, 33124);
#if 0
	convert2list();
